...
export default function ComparePage() {
      ...
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="glass-card p-8 text-center max-w-md">
          <h1 className="font-display text-2xl font-bold mb-4">
            Ingen institutioner valgt
          </h1>
          <p className="text-muted mb-6">
            Vælg mindst 2 institutioner for at sammenligne dem.
          </p>
          <Link
            to="/"
  ...
  const rateData = institutions.map((inst) => ({
    ...
    "Månedlig takst": inst.monthlyRate || 0,
  }));

  // Quality radar data (for schools)
  const hasQuality = institutions.some((i) => i.quality);
  const radarData = hasQuality
    ? [
        { metric: "Trivsel", ...Object.fromEntries(institutions.map((i, idx) => [`inst${idx}`, i.quality?.ts || 0])) },
        { metric: "Karakterer", ...Object.fromEntries(institutions.map((i, idx) => [`inst${idx}`, (i.quality?.k || 0) / 12 * 5])) },
        { metric: "Kompetence", ...Object.fromEntries(institutions.map((i, idx) => [`inst${idx}`, (i.quality?.kp || 0) / 100 * 5])) },
        { metric: "Lavt fravær", ...Object.fromEntries(institutions.map((i, idx) => [`inst${idx}`, i.quality?.fp ? (15 - i.quality.fp) / 15 * 5 : 0])) },
      ]
    : [];
      ...
      <section className="px-4 py-10 sm:py-14 bg-gradient-to-b from-primary/5 to-transparent">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-muted">
            {institutions.length} institutioner sammenlignet
          </p>
        </div>
      </section>
        ...
        <div className="glass-card p-4" style={{ height: 300 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={rateData} margin={{ top: 10, right: 20, bottom: 40, left: 20 }}>
              ...
              <Tooltip
                formatter={(value) => [formatDKK(Number(value)), "Takst"]}
                contentStyle={{ borderRadius: 12, border: "1px solid #E8DFD7" }}
              />
              <Bar dataKey="Månedlig takst" fill="#0E7490" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </section>
